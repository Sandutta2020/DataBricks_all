...
# MAGIC %md
# MAGIC ---
# MAGIC ## Comparing Sampling Strategies: Grid Search and Random Search
# MAGIC
# MAGIC Optuna's default sampler is **TPE (Tree-structured Parzen Estimator)**, a Bayesian method that
# MAGIC learns from past trials to focus on promising regions. Optuna also supports two classical
# MAGIC strategies via built-in samplers:
# MAGIC
# MAGIC | Sampler | Strategy | Best for |
# MAGIC |---|---|---|
# MAGIC | `TPESampler` *(default)* | Bayesian (model-based) | Large search spaces, continuous params |
# MAGIC | `GridSampler` | Exhaustive grid — tries every combination | Small discrete search spaces |
# MAGIC | `RandomSampler` | Uniform random draw | Baselines, fast exploration |
# MAGIC
# MAGIC The next two cells run short studies using each sampler so you can compare their trial histories
# MAGIC in the Optuna visualizations.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Grid Search with `GridSampler`
# MAGIC
# MAGIC `GridSampler` requires an explicit dictionary of candidate values for every parameter.
# MAGIC Optuna iterates through all combinations in the Cartesian product — identical to sklearn's
# MAGIC `GridSearchCV` — but still logs each trial to MLflow automatically.
...
with mlflow.start_run(run_name="demo_optuna_grid_search") as grid_run:
    grid_study.optimize(optuna_objective_function, n_trials=54)  # 2×3×3×3 = 54 combos
...
# MAGIC %md
# MAGIC ### Random Search with `RandomSampler`
# MAGIC
# MAGIC `RandomSampler` draws hyperparameter values uniformly at random from the same search space
# MAGIC defined in the objective function. It serves as a strong baseline: when random search matches
# MAGIC or beats TPE on small trial budgets, it signals the search space may be relatively flat.
...
# MAGIC %md
# MAGIC ### Compare Sampler Results
# MAGIC
# MAGIC The cell below summarises the best negative-F1 value achieved by each of the three
# MAGIC strategies in this demo so you can quickly gauge their relative performance.
