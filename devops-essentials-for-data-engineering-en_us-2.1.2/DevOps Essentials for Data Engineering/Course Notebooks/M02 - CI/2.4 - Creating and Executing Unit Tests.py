...
# MAGIC %md
# MAGIC Developing good unit tests is important because they help ensure individual components of your code work as expected, catching errors early in the development process. This leads to more reliable, maintainable code and can save time in debugging later on.
...
# MAGIC
# MAGIC          - `test_high_cholest_column_valid_map`
# MAGIC
# MAGIC          - `test_age_group_column_valid_map`
# MAGIC
...
# MAGIC
# MAGIC     - `../../tests/unit_tests/test_spark_helper_functions.py` - The path to the test file you want to run.
# MAGIC
...
# Reload the test module to ensure latest changes are picked up before running tests
importlib.reload(importlib.import_module('tests.unit_tests.test_spark_helper_functions'))

# Run pytest on the specified test file with verbose output
