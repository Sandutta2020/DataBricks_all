...
# MAGIC
# MAGIC In Databricks, you have several options for deploying your Databricks Assets such as the UI, REST APIs, Databricks CLI, Databricks SDK or Declarative Automation Bundles (DABs). Databricks recommends Declarative Automation Bundles for creating, developing, deploying, and testing jobs and other Databricks resources as source code. 
# MAGIC
...
# MAGIC %md
# MAGIC ## B. Create the Databricks Job with Notebooks, Python Files and a Lakeflow Spark Declarative Pipeline
# MAGIC
# MAGIC 1. In this section, we will create the job for our project. The job will contain the following tasks:
# MAGIC     - Unit tests
# MAGIC     - Spark Declarative Pipeline with the ETL pipeline and integration tests 
...
# MAGIC %md
# MAGIC 2. During development, it's beneficial to build out your Workflow and/or Lakeflow Spark Declarative Pipeline using the UI. The UI provides an easy way to build your desired job, and it generates the necessary JSON or YAML files to start automating your deployment across different environments.
# MAGIC
...
# MAGIC
# MAGIC    **NOTE:** If the Lakeflow Spark Declarative Pipeline has already completed, simply select the pipeline link.
# MAGIC
# MAGIC - c. Notice the pipeline executes (will execute) the ETL pipeline we created earlier.
# MAGIC
# MAGIC - d. Close the Spark Declarative Pipeline.
# MAGIC
# MAGIC
...
# MAGIC
# MAGIC    d. Notice that you can view job as the YAML for the job. This is a great way to easily obtain the necessary values for the YAML deployment (this YAML file is extremely helpful when deploying using **Declarative Automation Bundles**).
# MAGIC
...
# MAGIC - How do I configure the necessary variables for the Lakeflow Spark Declarative Pipeline during each deployment?
...
# MAGIC ### Next Steps: 
# MAGIC
# MAGIC In this course, you explored the essentials of CI/CD with a focus on continuous integration (CI) in Databricks. Next, you’ll want to dive into the other half of the DevOps pipeline, continuous deployment (CD) by learning how to deploy assets using **Declarative Automation Bundles (DABs)**.
# MAGIC
# MAGIC #### DABs Documentation [What are Declarative Automation Bundles?](https://docs.databricks.com/en/dev-tools/bundles/index.html) 
# MAGIC
# MAGIC #### Databricks Academy Course: [Automated Deployment with Declarative Automation Bundles](https://www.databricks.com/training/catalog/automated-deployment-with-declarative-automation-bundles-3724)

# COMMAND ----------

# MAGIC %md
# MAGIC # Next steps for Python unit testing in Databricks 
# MAGIC
# MAGIC You have now:
# MAGIC - **Organize a Databricks project** into `src/` and `tests/` folders with a `pyproject.toml` configuration file.
# MAGIC - **Write pure Python helper functions** and unit tests using `pytest` and `@pytest.mark.parametrize`.
# MAGIC - **Write PySpark transformation functions** and test them using a shared `SparkSession` fixture from `conftest.py`.
# MAGIC - **Run unit tests** interactively in the Databricks workspace UI and from a notebook cell.
# MAGIC - **Detect and fix bugs** in transformation logic using automated unit tests.
# MAGIC
# MAGIC ### Where to go next: 
# MAGIC
# MAGIC - Run `pytest` as a **Databricks Workflow task** for CI/CD integration
# MAGIC - Explore **Declarative Automation Bundles** to package tests alongside pipelines
# MAGIC - Review the Databricks documentation on [Python unit testing](https://docs.databricks.com/aws/en/files/python-unit-tests) in the workspace for more details 
# MAGIC
