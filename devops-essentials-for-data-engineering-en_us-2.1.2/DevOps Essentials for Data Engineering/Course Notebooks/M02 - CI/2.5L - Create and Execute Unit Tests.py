...
# MAGIC %skip
# MAGIC from src_lab.lab_functions import transforms
...
# MAGIC %skip
# MAGIC def test_uppercase_columns_function():
# MAGIC
# MAGIC     ## Fake DataFrame with random column names
# MAGIC     data = [(1, 5.0, 1, 1, 1, 1)]
# MAGIC     columns = ["id", "trip_distance", "My_Column", "WithNumbers123", "WithSymbolX@#", "With Space"]
# MAGIC     df = spark.createDataFrame(data, columns)
# MAGIC
# MAGIC     ## Apply the transforms.uppercase_columns_names function to return the actual column names
# MAGIC     actual_df = transforms.uppercase_columns_names(df)
# MAGIC     actual_columns = actual_df.columns
# MAGIC
# MAGIC     ## Create a list of the expected column names
# MAGIC     expected_columns = ['ID', 'TRIP_DISTANCE', 'MY_COLUMN', 'WITHNUMBERS123', 'WITHSYMBOLX@#', "WITH SPACE"]
# MAGIC
# MAGIC     ## Perform a test of the actual columns names and expected column names using a simple python assert statement
# MAGIC     assert actual_columns == expected_columns
# MAGIC     print('Test Passed!')
# MAGIC
# MAGIC test_uppercase_columns_function()
...
# MAGIC
# MAGIC from pyspark.sql import functions as F
# MAGIC from pyspark.sql.types import StructType, StructField, DoubleType
# MAGIC from pyspark.testing.utils import assertDataFrameEqual
# MAGIC
# MAGIC
# MAGIC def test_convert_miles_to_km_function():
# MAGIC     # Prepare a DataFrame with sample data
# MAGIC     data = [(1.0,), (5.5,), (None,)]
# MAGIC     schema = StructType([
# MAGIC         StructField("trip_distance_miles", DoubleType(), True)  # Allow null values by setting nullable=True
# MAGIC     ])
# MAGIC     actual_df = spark.createDataFrame(data, schema)
# MAGIC
# MAGIC
# MAGIC     ## Apply the function on the sample data and store the actual DataFrame
# MAGIC     actual_df = transforms.convert_miles_to_km(df = actual_df, 
# MAGIC                                                new_column_name="trip_distance_km",   ## Name of the new column
# MAGIC                                                miles_column="trip_distance_miles")   ## Name of the source miles column
# MAGIC
# MAGIC
# MAGIC     ## Create an expected DataFrame with a defined schema using StructField DoubleType for each column
# MAGIC     data = [
# MAGIC         (1.0, 1.61),   # Row with values
# MAGIC         (5.5, 8.85),   # Row with values
# MAGIC         (None, None) # Row with null values
# MAGIC     ]
# MAGIC
# MAGIC     ## Define schema
# MAGIC     schema = StructType([
# MAGIC         StructField("trip_distance_miles", DoubleType(), True),
# MAGIC         StructField("trip_distance_km", DoubleType(), True)
# MAGIC     ])
# MAGIC
# MAGIC     ## Create expected DataFrame
# MAGIC     expected_df = spark.createDataFrame(data, schema)
# MAGIC
# MAGIC
# MAGIC     ## Compare the actual and expected DataFrames using assertDataFrameEqual
# MAGIC     assertDataFrameEqual(actual_df, expected_df)
# MAGIC     print('Test Passed!')
# MAGIC
# MAGIC
# MAGIC ## Run the unit test
# MAGIC test_convert_miles_to_km_function()
...
# MAGIC
# MAGIC retcode = pytest.main(["./tests_lab/lab_unit_test_solution.py", "-v", "-p", "no:cacheprovider"])
# MAGIC
