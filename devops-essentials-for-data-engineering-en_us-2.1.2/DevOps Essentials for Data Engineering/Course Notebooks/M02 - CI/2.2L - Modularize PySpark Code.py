...
# MAGIC %skip
# MAGIC ## Load table
# MAGIC df = load_data("samples.nyctaxi.trips")
# MAGIC
# MAGIC ## Convert miles to km
# MAGIC df = convert_miles_to_km(df, new_column_name = "trip_distance_km", miles_column = "trip_distance")
# MAGIC
# MAGIC ## Upcase column
# MAGIC df = uppercase_columns_names(df)
# MAGIC
# MAGIC ## Save DataFrame as a table in your catalog
# MAGIC save_to_catalog(df, catalog_name = DA.catalog_name, schema_name="default", table_name = "my_lab_table")

# COMMAND ----------

# MAGIC %md
# MAGIC 3. Run the following cell to test that the original table created in cell 11 (**nyc_lab_solution_table**) is the same as your new table created by the functions above (**my_lab_table**). The test uses the PySpark `assertDataFrameEqual` method.
# MAGIC
